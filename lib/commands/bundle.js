...
async function bundle(argv) {
  ...
  if (mipFeatures['bundle.enabled'] === false) {
    console.log('ℹ️ Bundle command is disabled (bundle.enabled: false)');
    return;
  }
  ...
  if (mipFeatures['interactive.promptOnBundle'] !== false) {
    ...
    const answer = await new Promise(resolve => {
      ...
      rl.question(`📦 Bundle "${entry}"? (Y/n) `, resolve);
    });
    rl.close();
  ...
  if (mipFeatures['bundle.useCache'] !== false) {
    console.log(chalk.gray('   Using cache for faster builds'));
  }
  ...
  try {
    ...
    const buildOptions = {
      ...
      outfile: outfile,
      minify: options.minify || mipFeatures['bundle.minify'] || false,
      sourcemap: options.sourcemap || mipFeatures['bundle.sourcemap'] || false,
      external: options.external || mipFeatures['bundle.external'] || [],
      format: options.format || mipFeatures['bundle.format'] || 'cjs',
      mainFields: ['module', 'main'],
      banner: {
        js: '#!/usr/bin/env node\n'
      },
      metafile: true,
      logLevel: mipFeatures['bundle.logLevel'] || 'info',
      loader: {
        '.node': 'file'
      }
    };

    // Добавляем опции из фич
    if (mipFeatures['bundle.keepNames']) {
      buildOptions.keepNames = true;
    }
    if (mipFeatures['bundle.treeShaking']) {
    ...
    const maxSize = mipFeatures['bundle.maxSize'] || 0;
    if (maxSize > 0 && stats.totalBytes > maxSize * 1024 * 1024) {
      console.log(chalk.yellow(`⚠️ Bundle size exceeds limit (${maxSize} MB)`));
    }
    
    if (options.analyze || mipFeatures['bundle.analyze']) {
      console.log('');
      console.log(chalk.blue('📊 Bundle analysis:'));
      console.log(chalk.gray('   ─────────────────────────────'));
      const sorted = Object.entries(result.metafile.inputs)
        .sort((a, b) => b[1].bytes - a[1].bytes)
        .slice(0, mipFeatures['bundle.analyzeLimit'] || 10);
      
      for (const [file, info] of sorted) {
        const size = formatSize(info.bytes);
        const percentage = ((info.bytes / stats.totalBytes) * 100).toFixed(1);
        console.log(chalk.gray(`   ${size}  ${path.basename(file)} (${percentage}%)`));
      }
    }
    ...
    if (mipFeatures['bundle.showErrors'] !== false) {
...
function getStats(metafile) {
  ...
  for (const [_file, info] of Object.entries(inputs)) {
...
${chalk.bold('FEATURES')}
  bundle.enabled        - Enable/disable bundle command
  bundle.minify         - Minify output by default
  bundle.sourcemap      - Generate sourcemap by default
  bundle.format         - Default output format
  bundle.maxSize        - Warn if bundle exceeds size (MB)
  bundle.analyze        - Show bundle analysis by default
  bundle.useCache       - Use cache for faster builds
