...
async function repo(arg, opts = {}) {
  ...
  async function refreshLeft() {
    ...
    state.leftSelectedIndex = clamp(state.leftSelectedIndex, 0, Math.max(0, state.leftListing.length - 1));
  ...
  function render() {
    ...
    write('│' + padRight(leftTitle, leftWidth) + '│' + padRight(rightTitleLine, rightWidth) + '│' + os.EOL);
  ...
  function onData(buf) {
    ...
    if (s === 'j' || s === '\u001b[B') {
      state.leftSelectedIndex = clamp(state.leftSelectedIndex + 1, 0, Math.max(0, state.leftListing.length - 1));
      // keep selection visible
      if (!state.leftFocus) {
