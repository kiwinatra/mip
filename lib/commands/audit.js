/*
 * ┌─────────────────────────────────────────────────────────────────────┐
 * │   MInimal Package Manager                                          │
 * │   https://github.com/kiwinatra/mip                                 │
 * │   MIT License · Copyright (c) 2026 kiwinatra                        │
 * └─────────────────────────────────────────────────────────────────────┘
 */

const axios = require('axios');
const fs = require('fs');
const path = require('path');
const semver = require('semver');
const { execSync } = require('child_process');
const yaml = require('js-yaml');
const { loadLangForCwd, getI18n } = require('../i18n');
const config = require('../utils/config');
const features = require('../utils/features');
const log = require('../utils/log');

// ==========================================
// ЦВЕТА ДЛЯ АУДИТА
// ==========================================

const COLORS = {
  reset: '\x1b[0m',
  green: '\x1b[32m',
  yellow: '\x1b[33m',
  red: '\x1b[31m',
  blue: '\x1b[36m',
  gray: '\x1b[90m',
  bold: '\x1b[1m',
};

function colorize(text, color) {
  return `${COLORS[color]}${text}${COLORS.reset}`;
}

function getSeverityIcon(severity) {
  const icons = {
    critical: '🔴',
    high: '🟠',
    moderate: '🟡',
    low: '🔵',
  };
  return icons[severity] || '⚪';
}

function getSeverityColor(severity) {
  const colors = {
    critical: 'red',
    high: 'yellow',
    moderate: 'blue',
    low: 'green',
  };
  return colors[severity] || 'gray';
}

async function audit(options = {}) {
  const { fix = false } = options;
  const { t } = getI18n(loadLangForCwd(process.cwd()));
  const mipFeatures = features.loadFeatures(process.cwd());

  log.debug('Audit started', { options, fix, cwd: process.cwd() }, { showFile: true });

  // Проверка включен ли аудит
  if (mipFeatures['audit.enabled'] === false) {
    console.log('ℹ️ Audit is disabled (audit.enabled: false)');
    console.log('   Enable it in mip.config.yml or with: mip config set features.audit.enabled true');
    log.warn('Audit is disabled in config', { showFile: true });
    return;
  }

  // Проверка интерактивного режима
  if (mipFeatures['interactive.promptOnAudit'] !== false) {
    log.debug('Interactive prompt for audit enabled', { showFile: true });
    const readline = require('readline');
    const rl = readline.createInterface({
      input: process.stdin,
      output: process.stdout
    });
    
    const answer = await new Promise(resolve => {
      rl.question('🔍 Run security audit? (Y/n) ', resolve);
    });
    rl.close();
    
    if (answer.toLowerCase() === 'n' || answer.toLowerCase() === 'no') {
      console.log('❌ Audit cancelled');
      log.info('Audit cancelled by user', { showFile: true });
      return;
    }
  }

  // Проверяем флаг --output
  const outputArgIndex = process.argv.indexOf('--output');
  let outputFile = null;
  if (outputArgIndex !== -1) {
    outputFile = process.argv[outputArgIndex + 1];
    log.debug(`Output file specified: ${outputFile}`, { showFile: true });
  }

  // Проверяем флаг --json
  const isJson = process.argv.includes('--json');
  log.debug(`JSON output: ${isJson}`, { showFile: true });

  console.log(colorize('🔍 Running security audit...', 'bold'));
  log.info('Running security audit', { showFile: true });

  // Проверяем lock-файлы (YAML приоритет)
  const yamlLockPath = path.join(process.cwd(), 'mip-lock.yml');
  const jsonLockPath = path.join(process.cwd(), 'mip-lock.json');
  let lockData = null;
  let lockPath = null;

  if (fs.existsSync(yamlLockPath)) {
    lockPath = yamlLockPath;
    log.debug(`Found YAML lock file: ${lockPath}`, { showFile: true });
    try {
      lockData = yaml.load(fs.readFileSync(lockPath, 'utf8'));
    } catch (err) {
      console.log(t('commands.audit.lock_corrupted', { path: lockPath }));
      log.error(`Lock file corrupted: ${lockPath}`, { showFile: true });
      return;
    }
  } else if (fs.existsSync(jsonLockPath)) {
    lockPath = jsonLockPath;
    log.debug(`Found JSON lock file: ${lockPath}`, { showFile: true });
    try {
      lockData = JSON.parse(fs.readFileSync(lockPath, 'utf8'));
    } catch (err) {
      console.log(t('commands.audit.lock_corrupted', { path: lockPath }));
      log.error(`Lock file corrupted: ${lockPath}`, { showFile: true });
      return;
    }
  }

  if (!lockData) {
    console.log(t('commands.audit.lock_not_found'));
    log.warn('No lock file found', { showFile: true });
    return;
  }

  // Игнорирование dev зависимостей
  let packages = Object.entries(lockData.packages || {});
  
  if (mipFeatures['audit.ignoreDev']) {
    const conf = config.readConfig(process.cwd());
    const devDeps = conf?.devDependencies || {};
    packages = packages.filter(([fullName]) => {
      const name = fullName.split('@')[0];
      return !devDeps[name];
    });
    console.log('ℹ️ Dev dependencies excluded from audit');
    log.debug(`Dev dependencies excluded, ${packages.length} packages remaining`, { showFile: true });
  }

  if (packages.length === 0) {
    console.log(t('commands.audit.no_packages'));
    log.info('No packages to audit', { showFile: true });
    return;
  }

  console.log(t('commands.audit.scanning', { count: packages.length }));
  log.debug(`Scanning ${packages.length} packages`, { showFile: true });

  const vulnerabilities = [];
  const fixable = [];

  // Параллельное сканирование
  const useParallel = mipFeatures['audit.parallelScan'] !== false;
  const timeout = mipFeatures['audit.timeout'] || 5000;
  log.debug(`Parallel scan: ${useParallel}, timeout: ${timeout}ms`, { showFile: true });

  if (useParallel && packages.length > 1) {
    const chunkSize = mipFeatures['performance.parallelDownloads'] || 5;
    const chunks = [];
    for (let i = 0; i < packages.length; i += chunkSize) {
      chunks.push(packages.slice(i, i + chunkSize));
    }
    log.debug(`Processing ${chunks.length} chunks of size ${chunkSize}`, { showFile: true });
    
    for (const chunk of chunks) {
      const promises = chunk.map(async ([fullName, info]) => {
        const name = fullName.split('@')[0];
        try {
          const response = await axios.get(
            `https://registry.npmjs.org/-/npm/v1/security/advisories?package=${name}`,
            { timeout, validateStatus: () => true }
          );
          return { name, fullName, info, data: response.data };
        } catch (err) {
          log.debug(`Failed to fetch advisories for ${name}: ${err.message}`, { showFile: true });
          return { name, fullName, info, data: null, error: err };
        }
      });
      
      const results = await Promise.all(promises);
      
      for (const result of results) {
        if (result.data) {
          processAdvisories(result, vulnerabilities, fixable, fix, mipFeatures);
        }
      }
    }
  } else {
    for (const [fullName, info] of packages) {
      const name = fullName.split('@')[0];
      try {
        const response = await axios.get(
          `https://registry.npmjs.org/-/npm/v1/security/advisories?package=${name}`,
          { timeout, validateStatus: () => true }
        );
        processAdvisories({ fullName, info, data: response.data }, vulnerabilities, fixable, fix, mipFeatures);
      } catch (err) {
        console.log('ERR_NO_CONNECTION \n');
        log.warn(`Failed to fetch for ${name}: ${err.message}`, { showFile: true });
      }
    }
  }

  log.debug(`Found ${vulnerabilities.length} vulnerabilities, ${fixable.length} fixable`, { showFile: true });

  // ==========================================
  // ФОРМИРУЕМ РЕЗУЛЬТАТ
  // ==========================================

  const result = {
    timestamp: new Date().toISOString(),
    summary: {
      total: vulnerabilities.length,
      critical: vulnerabilities.filter(v => v.severity === 'critical').length,
      high: vulnerabilities.filter(v => v.severity === 'high').length,
      moderate: vulnerabilities.filter(v => v.severity === 'moderate').length,
      low: vulnerabilities.filter(v => v.severity === 'low').length,
    },
    vulnerabilities: vulnerabilities,
    fixable: fixable,
  };

  // ==========================================
  // ВЫВОД В ФАЙЛ (если указан --output)
  // ==========================================

  if (outputFile) {
    try {
      const outputPath = path.isAbsolute(outputFile)
        ? outputFile
        : path.join(process.cwd(), outputFile);
      fs.writeFileSync(outputPath, JSON.stringify(result, null, 2));
      console.log(colorize(`📄 Report saved to: ${outputPath}`, 'green'));
      log.success(`Audit report saved to ${outputPath}`, { showFile: true });
    } catch (err) {
      console.log(colorize(`❌ Failed to save report: ${err.message}`, 'red'));
      log.error(`Failed to save report: ${err.message}`, { showFile: true });
    }
  }

  // ==========================================
  // ВЫВОД В КОНСОЛЬ (JSON или цветной)
  // ==========================================

  if (isJson) {
    console.log(JSON.stringify(result, null, 2));
    log.debug('Audit results output as JSON', { showFile: true });
    return;
  }

  if (vulnerabilities.length === 0) {
    console.log(colorize('✅ No vulnerabilities found', 'green'));
    log.success('No vulnerabilities found', { showFile: true });
    return;
  }

  vulnerabilities.sort((a, b) => b.cvss_score - a.cvss_score);

  const critical = vulnerabilities.filter(v => v.severity === 'critical');
  const high = vulnerabilities.filter(v => v.severity === 'high');
  const moderate = vulnerabilities.filter(v => v.severity === 'moderate');
  const low = vulnerabilities.filter(v => v.severity === 'low');

  console.log(colorize('\n📋 Vulnerabilities found:', 'bold'));

  [...critical, ...high, ...moderate, ...low].forEach(v => {
    const icon = getSeverityIcon(v.severity);
    const color = getSeverityColor(v.severity);
    const severityLabel = v.severity.toUpperCase();

    console.log(colorize(`\n${icon} ${v.package}`, color));
    console.log(`   ${colorize('Severity:', 'bold')} ${colorize(severityLabel, color)}`);
    console.log(`   ${colorize('Title:', 'bold')} ${v.title}`);
    console.log(`   ${colorize('CVSS:', 'bold')} ${v.cvss_score}`);
    console.log(
      `   ${colorize('Fix:', 'bold')} ${v.patched_versions || colorize('Update manually', 'yellow')}`
    );
    console.log(`   ${colorize('URL:', 'bold')} ${v.url}`);
  });

  console.log(colorize('\n📊 Summary:', 'bold'));
  console.log(`   ${colorize('🔴 Critical:', 'red')} ${critical.length}`);
  console.log(`   ${colorize('🟠 High:', 'yellow')} ${high.length}`);
  console.log(`   ${colorize('🟡 Moderate:', 'blue')} ${moderate.length}`);
  console.log(`   ${colorize('🔵 Low:', 'green')} ${low.length}`);

  if (outputFile) {
    console.log(colorize(`\n📄 Report saved to: ${outputFile}`, 'green'));
  }

  const shouldFix = fix || mipFeatures['audit.fixAutomatically'];

  if (shouldFix && fixable.length > 0) {
    console.log(colorize('\n🔧 Fixing vulnerabilities...', 'bold'));
    log.info(`Fixing ${fixable.length} vulnerabilities`, { showFile: true });

    for (const pkg of fixable) {
      console.log(
        colorize(
          `  Updating ${pkg.name}: ${pkg.currentVersion} → ${pkg.fixedVersion}`,
          'yellow'
        )
      );
      log.debug(`Updating ${pkg.name} from ${pkg.currentVersion} to ${pkg.fixedVersion}`, { showFile: true });

      try {
        execSync(`mip install ${pkg.name}@${pkg.fixedVersion}`, {
          stdio: 'pipe',
        });

        const conf = config.readConfig(process.cwd());
        if (conf) {
          if (!conf.dependencies) conf.dependencies = {};
          if (!conf.devDependencies) conf.devDependencies = {};

          if (conf.dependencies[pkg.name] !== undefined) {
            conf.dependencies[pkg.name] = pkg.fixedVersion;
          } else if (conf.devDependencies[pkg.name] !== undefined) {
            conf.devDependencies[pkg.name] = pkg.fixedVersion;
          } else {
            conf.dependencies[pkg.name] = pkg.fixedVersion;
          }

          config.writeConfig(conf, process.cwd());
        }

        console.log(colorize(`  ✅ ${pkg.name} updated`, 'green'));
        log.success(`Updated ${pkg.name} to ${pkg.fixedVersion}`, { showFile: true });
      } catch (err) {
        console.log(colorize(`  ❌ Failed to update ${pkg.name}: ${err.message}`, 'red'));
        log.error(`Failed to update ${pkg.name}: ${err.message}`, { showFile: true });
      }
    }

    console.log(colorize('\n✅ Fix complete', 'green'));
    console.log(colorize('🔄 Run mip audit again to verify', 'gray'));
    log.success('Fix completed', { showFile: true });
  } else if (shouldFix && fixable.length === 0 && vulnerabilities.length > 0) {
    console.log(colorize('\n⚠️ No packages can be auto-fixed', 'yellow'));
    log.info('No packages can be auto-fixed', { showFile: true });
  } else if (vulnerabilities.length > 0) {
    console.log(colorize('\n💡 Run mip audit --fix to automatically update', 'yellow'));
    log.info('Vulnerabilities found, suggest --fix', { showFile: true });
  }

  log.debug('Audit finished', { showFile: true });
}

function processAdvisories(result, vulnerabilities, fixable, fix, mipFeatures) {
  const { fullName, info, data } = result;
  const name = fullName.split('@')[0];
  
  const advisories = data?.objects || [];

  for (const adv of advisories) {
    if (semver.satisfies(info.version, adv.vulnerable_versions)) {
      const severity = adv.severity || 'moderate';
      
      // Фильтрация по уровню
      if (severity === 'low' && mipFeatures['audit.ignoreLow']) continue;
      if (severity === 'moderate' && mipFeatures['audit.ignoreModerate']) continue;
      if (severity === 'high' && mipFeatures['audit.ignoreHigh']) continue;
      if (severity === 'critical' && mipFeatures['audit.ignoreCritical']) continue;

      const vuln = {
        package: fullName,
        name: name,
        version: info.version,
        severity: severity,
        title: adv.title,
        url: adv.url,
        cvss_score: adv.cvss?.score || 0,
        vulnerable_versions: adv.vulnerable_versions,
        patched_versions: adv.patched_versions,
      };
      vulnerabilities.push(vuln);
      log.debug(`Vulnerability found: ${fullName} (${severity})`, { showFile: true });

      if (fix && adv.patched_versions) {
        const fixedVersion = findFixableVersion(info.version, adv.patched_versions);
        if (fixedVersion && fixedVersion !== info.version) {
          fixable.push({
            name: name,
            currentVersion: info.version,
            fixedVersion: fixedVersion,
            severity: vuln.severity,
          });
          log.debug(`Fixable: ${name} -> ${fixedVersion}`, { showFile: true });
        }
      }
    }
  }
}

function findFixableVersion(currentVersion, patchedVersions) {
  const patchedList = patchedVersions.split(' ').filter(v => v.trim());

  for (const patched of patchedList) {
    if (semver.valid(patched) && semver.gt(patched, currentVersion)) {
      const parsed = semver.parse(patched);
      const current = semver.parse(currentVersion);

      if (parsed && current && parsed.major === current.major) {
        return patched;
      }
    }
  }

  return null;
}

module.exports = { audit };