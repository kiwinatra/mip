  ...
  if (fix && fixable.length > 0) {
    ...
    for (const pkg of fixable) {
      ...
      try {
        ...
        // Sync mip.json too (issue #14)
        const mipJsonPath = path.join(process.cwd(), 'mip.json');
        if (fs.existsSync(mipJsonPath)) {
          const config = JSON.parse(fs.readFileSync(mipJsonPath, 'utf8'));
          if (!config.dependencies) config.dependencies = {};
          if (!config.devDependencies) config.devDependencies = {};

          if (config.dependencies[pkg.name] !== undefined) {
            config.dependencies[pkg.name] = pkg.fixedVersion;
          } else if (config.devDependencies[pkg.name] !== undefined) {
            config.devDependencies[pkg.name] = pkg.fixedVersion;
          } else {
            // Default to dependencies when package was not present before
            config.dependencies[pkg.name] = pkg.fixedVersion;
          }

          fs.writeFileSync(mipJsonPath, JSON.stringify(config, null, 2));
        }
