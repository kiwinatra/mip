...
function isGlobalPluginMode() {
  // Default behavior: create plugins into the global cache root.
  // This matches how other plugins are discovered by PluginManager.
  // Users can still opt-in local creation by setting:
  //   MIP_PLUGINS_LOCAL=1
  const argv = process.argv || [];
  const forceLocal = process.env.MIP_PLUGINS_LOCAL === '1' || argv.includes('--local');
  return !forceLocal;
}

function getGlobalPluginsDir() {
  // PluginManager.loadAll() loads cached plugins from:
  //   ~/.mip_cache/plugins/<pluginName>/index.js
  return getPluginCacheDir();
}



function createPlugin(pluginName) {
  const pluginsDir = isGlobalPluginMode() ? getGlobalPluginsDir() : getProjectPluginsDir();
