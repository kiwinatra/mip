...
async function repo(arg, opts = {}) {
  ...
  function printTree(listing) {
    ...
    const files = listing.filter(x => x.type === 'file').map(x => x.name);
  ...
  function enterFullscreenReadme(lines, { title } = {}) {
    ...
    const draw = () => {
      ...
      const headerText =
        title ||
        `mip repo • ${state.owner}/${state.repository}${state.cwdPath ? `/${state.cwdPath}` : ''} • readme • q`;
  ...
  async function renderCurrentView(showContent = false) {
    ...
    try {
      ...
      console.log(
        ui.error(t('commands.repo.error.cannot_open_dir', { path: state.cwdPath || '/' }))
      );
      return;
    }
    ...
    console.log(
      ui.header(
        `mip repo: ${state.owner}/${state.repository}${prettyPath === '/' ? '' : `/${prettyPath}`}`
      )
    );
  ...
  async function handleGet(filename) {
    ...
    const relative = state.cwdPath ? `${state.cwdPath.replace(/\/+$/g, '')}/${safeName}` : safeName;
    ...
    console.log(
      ui.success(
        t('commands.repo.get.saved', { filename, path: path.relative(process.cwd(), outPath) })
      )
    );
  }
  ...
  rl.on('line', async line => {
    ...
    try {
      ...
      console.log(
        ui.error(
          t('commands.repo.error.generic', { message: e && e.message ? e.message : String(e) })
        )
      );
      rl.prompt();
    }
