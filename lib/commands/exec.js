...
function exec(command) {
  ...
  // Проверка unsafe (из фич)
  if (mipFeatures['exec.blockUnsafe'] !== false) {
  ...
  // Интерактивный режим
  if (mipFeatures['interactive.promptOnExec'] !== false) {
    ...
    const answer = new Promise(resolve => {
      rl.question(`⚡ Execute "${command}"? (Y/n) `, resolve);
    });
    rl.close();
