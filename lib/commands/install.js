...
async function installPackage(name, versionRange, options = {}) {
  ...
  const installDir = path.join(process.cwd(), '.mip', name, pkgInfo.version);
  ...
  // Обновляем lockfile
  updateLockfile(name, pkgInfo.version, pkgInfo.tarball, pkgInfo.dependencies);
...
function updateLockfile(name, version, resolved, dependencies) {
