...
const features = require('../utils/features'); // 👈 ДОБАВИТЬ
...
function configList(argv) {
  ...
  // 👇 ДОБАВИТЬ: ПРОВЕРКА НА ФИЧИ
  const showFeatures = argv.includes('--features') || argv.includes('-f');
  ...
  if (format === 'json') {
    if (showFeatures) {
      console.log(JSON.stringify({ config: conf, features: mipFeatures }, null, 2));
    } else {
  ...
  if (format === 'yaml') {
    try {
      ...
      if (showFeatures) {
        console.log(yaml.dump({ config: conf, features: mipFeatures }));
      } else {
  ...
  // 👇 ДОБАВИТЬ: ПОКАЗ ФИЧ
  if (showFeatures) {
    const enabled = mipFeatures['config.enabled'] !== false;
    console.log(chalk.bold(`🔧 Features ${enabled ? chalk.green('(enabled)') : chalk.red('(disabled)')}:`));
    
    const featureKeys = Object.keys(mipFeatures).sort();
    if (featureKeys.length === 0) {
      console.log(`   ${chalk.gray('No features loaded (config.enabled: false)')}`);
    } else {
      for (const key of featureKeys) {
        const value = mipFeatures[key];
        const status = value === true ? chalk.green('✅') : value === false ? chalk.red('❌') : chalk.gray('⚪');
        const isDefault = value === features.DEFAULT_FEATURES[key];
        console.log(`   ${status} ${chalk.cyan(key)}: ${chalk.yellow(value)}${isDefault ? chalk.gray(' (default)') : ''}`);
      }
    }
    console.log('');
  }
  ...
  console.log(chalk.gray(`   "mip config list --features" to view all features`));
}
...
function configGet(argv) {
  if (argv.length < 1) {
    ...
    console.error('   Example: mip config get features.config.enabled');
    process.exit(1);
  }
  ...
  if (key.startsWith('features.')) {
    ...
    const mipFeatures = features.loadFeatures(process.cwd());
    const value = mipFeatures[featureKey];
    if (value === undefined) {
      console.error(chalk.red(`❌ Feature "${featureKey}" not found`));
      process.exit(1);
    }
    console.log(value);
    return;
...
function configSet(argv) {
  if (argv.length < 2) {
    ...
    console.error('   Example: mip config set features.config.enabled true');
    process.exit(1);
  }
  ...
  if (key.startsWith('features.')) {
    ...
    const result = features.setFeature(featureKey, parsedValue, process.cwd());
    if (result) {
      console.log(chalk.green(`✅ Feature updated: ${chalk.cyan(featureKey)} = ${chalk.yellow(parsedValue)}`));
    } else {
      console.error(chalk.red(`❌ Failed to update feature: ${featureKey}`));
      process.exit(1);
    }
    return;
  }
...
function configDelete(argv) {
  if (argv.length < 1) {
    ...
    console.error('   Example: mip config delete features.config.enabled');
    process.exit(1);
  }
  ...
  if (key.startsWith('features.')) {
    const featureKey = key.replace('features.', '');
    const result = features.resetFeature(featureKey, process.cwd());
    if (result) {
      console.log(chalk.green(`✅ Feature reset to default: ${chalk.cyan(featureKey)}`));
    } else {
      console.error(chalk.red(`❌ Failed to reset feature: ${featureKey}`));
      process.exit(1);
    }
    return;
  }
...
${chalk.bold('OPTIONS')}
  ...
  --features, -f         Show features when using list

${chalk.bold('EXAMPLES')}
  ${chalk.gray('# Show all config')}
  mip config list

  ${chalk.gray('# Show config with features')}
  mip config list --features
  ...
  ${chalk.gray('# Get feature value')}
  mip config get features.config.enabled
  ...
  ${chalk.gray('# Enable features')}
  mip config set features.config.enabled true

  ${chalk.gray('# Disable all features')}
  mip config set features.config.enabled false

  ${chalk.gray('# Enable a specific feature')}
  mip config set features.install.parallel true

  ${chalk.gray('# Delete a value')}
  mip config delete registries.github.token

  ${chalk.gray('# Reset feature to default')}
  mip config delete features.install.parallel
...
${chalk.bold('SUPPORTED KEYS')}
  ...
  ${chalk.cyan('features.*')}        Feature flags (use with features. prefix)
