...
async function clone(argv) {
  ...
  if (mipFeatures['clone.enabled'] === false) {
    console.log('ℹ️ Clone command is disabled (clone.enabled: false)');
    return;
  }
...
async function cloneCreate(argv, lang) {
  ...
  if (mipFeatures['interactive.promptOnClone'] !== false) {
    ...
    const answer = await new Promise(resolve => {
      rl.question(`📦 Create clone snapshot? (Y/n) `, resolve);
    });
    rl.close();
  ...
  // Проверка на существующий снапшот
  if (fs.existsSync(outputPath) && mipFeatures['clone.overwrite'] === false) {
    console.log(chalk.yellow(`⚠️ Snapshot already exists at ${outputPath}`));
    console.log('   Use clone.overwrite: true to overwrite');
    return;
  }
  ...
  if (mipFeatures['clone.skipEmpty'] !== false) {
    const hasData = Object.keys(snapshot.dependencies).length > 0 || 
                    Object.keys(snapshot.devDependencies).length > 0 ||
                    Object.keys(snapshot.scripts).length > 0;
    if (!hasData) {
      console.log(chalk.yellow('⚠️ No data to snapshot (empty project)'));
      console.log('   Use clone.skipEmpty: false to create empty snapshot');
      return;
    }
  }
  ...
  try {
    ...
    if (mipFeatures['clone.saveHistory'] !== false) {
      const historyPath = path.join(distDir, 'mipclone-history.yml');
      let history = [];
      if (fs.existsSync(historyPath)) {
        try {
          history = yaml.load(fs.readFileSync(historyPath, 'utf8')) || [];
        } catch (e) {}
      }
      history.push({
      ...
      const maxHistory = mipFeatures['clone.maxHistory'] || 10;
      if (history.length > maxHistory) {
        history = history.slice(-maxHistory);
      }
      fs.writeFileSync(historyPath, yaml.dump(history, { indent: 2 }), 'utf8');
    }
  } catch (err) {
...
async function cloneView(argv, lang) {
  ...
  try {
    ...
    const _showAll = mipFeatures['clone.showAll'] !== false;
    ...
    if (data.dependencies) {
      ...
      const maxDisplay = mipFeatures['clone.maxDisplay'] || 10;
      const displayDeps = Object.entries(deps).slice(0, maxDisplay);
      for (const [name, version] of displayDeps) {
        console.log(`     ${name}@${version}`);
      }
      if (total > maxDisplay) console.log(`     ... and ${total - maxDisplay} more`);
      console.log('');
    }
    ...
    if (data.scripts) {
      ...
      const maxDisplay = mipFeatures['clone.maxDisplay'] || 5;
      const displayScripts = Object.entries(scripts).slice(0, maxDisplay);
      for (const [name, cmd] of displayScripts) {
...
async function cloneApply(argv, lang) {
  ...
  if (fs.existsSync(projectDir)) {
    ...
    if (mipFeatures['clone.autoOverwrite']) {
      console.log('   Auto-overwriting...');
  ...
  try {
    ...
    if (mipFeatures['clone.autoInstall']) {
      console.log('   Installing dependencies...');
      const { execSync } = require('child_process');
      try {
        execSync(`cd "${projectDir}" && mip install`, { stdio: 'inherit' });
      } catch (e) {
...
${chalk.bold('FEATURES')}
  clone.enabled          - Enable/disable clone command
  clone.overwrite        - Overwrite existing snapshot
  clone.skipEmpty        - Skip empty snapshots
  clone.saveHistory      - Save snapshot history
  clone.maxHistory       - Maximum history entries (default: 10)
  clone.maxDisplay       - Maximum items to display (default: 10)
  clone.autoOverwrite    - Auto-overwrite existing project
  clone.autoInstall      - Auto-install dependencies after apply
