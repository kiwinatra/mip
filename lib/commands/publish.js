...
async function publish(argv) {
  ...
  // Определяем реестр
  let registryName = options.registry || getDefaultRegistry(mipFeatures);
  
  // Проверка fallback
  const fallbackToNpm = mipFeatures['registry.fallbackToNpm'] !== false;
  let registryConfig = getRegistryConfig(registryName);

  if (!registryConfig && fallbackToNpm && registryName !== 'npm') {
    console.log(chalk.yellow(`⚠️ Registry "${registryName}" not found, falling back to npm`));
    registryName = 'npm';
    registryConfig = getRegistryConfig('npm');
  }
  ...
  try {
    await publishToRegistry(pkgInfo, registryConfig, registryName, options, mipFeatures);
 ...
 */
function getDefaultRegistry(mipFeatures) {
  const conf = config.readConfig(process.cwd());
  if (conf && conf.defaultRegistry) {
    return conf.defaultRegistry;
  }

  if (mipFeatures && mipFeatures['registry.default']) {
    return mipFeatures['registry.default'];
  }
 ...
 * Публикация в реестр
 */
async function publishToRegistry(pkgInfo, registryConfig, registryName, options, mipFeatures) {
  // Создаём tarball
...
${chalk.bold('FEATURES')}
  registry.default       - Default registry
  registry.fallbackToNpm - Fallback to npm if custom registry fails
