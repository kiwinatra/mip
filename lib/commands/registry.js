...
function registry(argv) {
  ...
  switch (subcommand) {
    ...
    case 'remove':
    case 'rm':
      return registryRemove(args);
    case 'list':
    case 'ls':
      return registryList(args);
    case 'set-default':
      return registrySetDefault(args);
    case 'help':
    case '--help':
    case '-h':
      return showHelp();
 ...
 */
function registryList(argv) {
  const globalRegistries = loadGlobalRegistries();
  const conf = config.readConfig(process.cwd());

  console.log(chalk.blue('📦 Configured registries:\n'));

  // Всегда показываем npm как базовый реестр
 ...
 */
function registrySetDefault(argv) {
  if (argv.length < 1) {
