 ...
 * │   https://github.com/meeple-packager/mip                          │
 * │   MIT License · Copyright (c) 2026 meeple-packager                │
 * └─────────────────────────────────────────────────────────────────────┘
 */
...
function configSet(argv) {
  ...
  // Сохраняем конфиг
  const configFile = configPath.path;
  if (typeof configFile !== 'string') {
    console.error(chalk.red('❌ Invalid config path'));
    process.exit(1);
  }

  try {
    const yaml = require('js-yaml');
    const content = yaml.dump(conf);
