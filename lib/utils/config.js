...
function readConfig(cwd = process.cwd()) {
  ...
  CONFIG_TTL = (mipFeatures['config.cacheTTL'] || 5) * 1000;
...
function writeConfig(content, cwd = process.cwd()) {
  const mipFeatures = features.loadFeatures(cwd);
  const useYaml = mipFeatures['config.useYaml'] !== false;
...
function migrateToYaml(cwd = process.cwd()) {
  const mipFeatures = features.loadFeatures(cwd);
  const autoMigrate = mipFeatures['config.autoMigrate'] !== false;
...
function migrateLockfile(cwd = process.cwd()) {
  const mipFeatures = features.loadFeatures(cwd);
  const autoMigrate = mipFeatures['config.autoMigrateLock'] !== false;
