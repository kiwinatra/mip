...
class ExportsResolver {
    ...
    if (typeof exportsConfig === 'object') {
      ...
      for (const condition of conditions) {
        if (exportsConfig[condition]) {
          const resolved = this.resolveExportPath(exportsConfig[condition], subpath, packagePath, packageJson);
      ...
      if (exportsConfig[subpath]) {
        return this.resolveExportPath(exportsConfig[subpath], subpath, packagePath, packageJson);
      }

      // Wildcard subpath (./*)
      if (exportsConfig['./*']) {
        const wildcardPath = subpath.replace(/^\.\//, '');
        return this.resolveExportPath(exportsConfig['./*'], wildcardPath, packagePath, packageJson);
      }
    }
  ...
  resolveExportPath(exportPath, subpath, packagePath, packageJson) {
    ...
    if (typeof exportPath === 'object') {
      // 🔥 ИСПРАВЛЕНО: передаём все аргументы правильно
