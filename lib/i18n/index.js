...
// ==========================================
// ДИНАМИЧЕСКАЯ ЗАГРУЗКА ВСЕХ ЛОКАЛЕЙ
// ==========================================
const LOCALES_DIR = path.join(__dirname, 'locales');
const LOCALES = {};

// Автоматически загружаем все .json из папки locales
(function loadAllLocales() {
  if (fs.existsSync(LOCALES_DIR)) {
    ...
    for (const file of files) {
      ...
      try {
        LOCALES[lang] = require(path.join(LOCALES_DIR, file));
      } catch (err) {
        // silently skip invalid locale files
      }
    }
  }
})();
