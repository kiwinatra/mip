...
class UnixBuilder {
  ...
  getShellConfig() {
    ...
    if (shell.includes('bash'))
      return { name: '.bashrc', path: path.join(this.homeDir, '.bashrc') };
    if (shell.includes('fish'))
      return {
        name: '.config/fish/config.fish',
        path: path.join(this.homeDir, '.config/fish/config.fish'),
      };

    if (fs.existsSync(path.join(this.homeDir, '.zshrc')))
      return { name: '.zshrc', path: path.join(this.homeDir, '.zshrc') };
    if (fs.existsSync(path.join(this.homeDir, '.bashrc')))
      return { name: '.bashrc', path: path.join(this.homeDir, '.bashrc') };
