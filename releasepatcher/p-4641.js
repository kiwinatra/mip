export class ProcessManager {
  ...
  attachEventHandlers(child, record) {
    ...
    child.on('exit', (code, signal) => {
      ...
      if (
        ...
        code !== 0 &&
        record.restartCount < this.config.retryAttempts
      ) {
  ...
  async sendMessage(name, message) {
    ...
    const serialized = this.config.serialization === 'json' ? JSON.stringify(message) : message;
  ...
  checkResourceLimits(name) {
    ...
    if (metrics.memory > this.config.maxMemoryRestart) {
      this.log(
        'warn',
        `Process ${name} exceeded memory limit: ${metrics.memory}MB / ${this.config.maxMemoryRestart}MB`
      );
    ...
    if (metrics.cpu > this.config.maxCpuRestart) {
      this.log(
        'warn',
        `Process ${name} exceeded CPU limit: ${metrics.cpu}% / ${this.config.maxCpuRestart}%`
      );
  ...
  async healthCheck() {
    ...
    for (const [name, record] of this.processes) {
      const isHealthy =
        record.status === 'running' &&
        (!this.metrics.get(name) || this.metrics.get(name).memory < this.config.maxMemoryRestart);
